/**
 * 服务:ArticleComment，文章的评论
 */
app
    .service('ArticleCommentService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("ArticleCommentService in");
        var aurl=ENV.api + "/ArticleComment";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj ArticleComment 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("ArticleCommentService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("ArticleCommentService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_ArticleComment);
            }
        };
}]);
