/**
 * 服务:QuickTxtComment，抢答的评价
 */
app
    .service('QuickTxtCommentService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("QuickTxtCommentService in");
        var aurl=ENV.api + "/QuickTxtComment";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj QuickTxtComment 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("QuickTxtCommentService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("QuickTxtCommentService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_QuickTxtComment);
            }
        };
}]);
