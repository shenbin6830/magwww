/**
 * 服务:ExpmemberStatiMonth，会员经验月统计
 */
app
    .service('ExpmemberStatiMonthService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("ExpmemberStatiMonthService in");
        var aurl=ENV.api + "/ExpmemberStatiMonth";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj ExpmemberStatiMonth 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("ExpmemberStatiMonthService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("ExpmemberStatiMonthService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_ExpmemberStatiMonth);
            }
        };
}]);
