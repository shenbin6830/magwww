/**
 * 服务:OrderrQuestionFinished，订单之一对一问题提问归档
 */
app
    .service('OrderrQuestionFinishedService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("OrderrQuestionFinishedService in");
        var aurl=ENV.api + "/OrderrQuestionFinished";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj OrderrQuestionFinished 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("OrderrQuestionFinishedService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("OrderrQuestionFinishedService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_OrderrQuestionFinished);
            }
        };
}]);
