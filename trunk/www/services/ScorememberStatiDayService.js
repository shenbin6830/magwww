/**
 * 服务:ScorememberStatiDay，会员积分日统计
 */
app
    .service('ScorememberStatiDayService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("ScorememberStatiDayService in");
        var aurl=ENV.api + "/ScorememberStatiDay";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj ScorememberStatiDay 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("ScorememberStatiDayService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("ScorememberStatiDayService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_ScorememberStatiDay);
            }
        };
}]);
