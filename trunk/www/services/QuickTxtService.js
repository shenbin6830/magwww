 ...
 */
app
    .service('QuickTxtService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("QuickTxtService in");
        var aurl=ENV.api + "/QuickTxt";
        return {
            /**
             * 初次查询
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("QuickTxtService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
