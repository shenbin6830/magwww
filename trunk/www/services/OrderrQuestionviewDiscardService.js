/**
 * 服务:OrderrQuestionviewDiscard，订单之一对一问题观看放弃
 */
app
    .service('OrderrQuestionviewDiscardService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("OrderrQuestionviewDiscardService in");
        var aurl=ENV.api + "/OrderrQuestionviewDiscard";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj OrderrQuestionviewDiscard 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("OrderrQuestionviewDiscardService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("OrderrQuestionviewDiscardService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_OrderrQuestionviewDiscard);
            }
        };
}]);
