/**
 * 服务:OrderrQuestionDiscard，订单之一对一问题提问放弃
 */
app
    .service('OrderrQuestionDiscardService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("OrderrQuestionDiscardService in");
        var aurl=ENV.api + "/OrderrQuestionDiscard";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj OrderrQuestionDiscard 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("OrderrQuestionDiscardService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("OrderrQuestionDiscardService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_OrderrQuestionDiscard);
            }
        };
}]);
