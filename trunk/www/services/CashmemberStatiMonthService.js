/**
 * 服务:CashmemberStatiMonth，会员现金月统计
 */
app
    .service('CashmemberStatiMonthService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("CashmemberStatiMonthService in");
        var aurl=ENV.api + "/CashmemberStatiMonth";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj CashmemberStatiMonth 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("CashmemberStatiMonthService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("CashmemberStatiMonthService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_CashmemberStatiMonth);
            }
        };
}]);
