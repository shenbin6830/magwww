/**
 * 服务:QuickLinkMemberView，观看抢答的会员
 */
app
    .service('QuickLinkMemberViewService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("QuickLinkMemberViewService in");
        var aurl=ENV.api + "/QuickLinkMemberView";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj QuickLinkMemberView 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("QuickLinkMemberViewService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("QuickLinkMemberViewService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_QuickLinkMemberView);
            }
        };
}]);
