/**
 * 服务:ScorememberStatiMonth，会员积分月统计
 */
app
    .service('ScorememberStatiMonthService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("ScorememberStatiMonthService in");
        var aurl=ENV.api + "/ScorememberStatiMonth";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj ScorememberStatiMonth 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("ScorememberStatiMonthService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("ScorememberStatiMonthService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_ScorememberStatiMonth);
            }
        };
}]);
