/**
 * 服务:QuestionAdd，一对一问题之追加
 */
app
    .service('QuestionAddService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("QuestionAddService in");
        var aurl=ENV.api + "/QuestionAdd";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj QuestionAdd 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("QuestionAddService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("QuestionAddService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
