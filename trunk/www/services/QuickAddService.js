 ...
 */
app
    .service('QuickAddService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("QuickAddService in");
        var aurl=ENV.api + "/QuickAdd";
        return {
            /**
             * 初次查询
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("QuickAddService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
