/**
 * 服务:OrderrQuickDiscard，订单之抢答问题提问放弃
 */
app
    .service('OrderrQuickDiscardService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("OrderrQuickDiscardService in");
        var aurl=ENV.api + "/OrderrQuickDiscard";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj OrderrQuickDiscard 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("OrderrQuickDiscardService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("OrderrQuickDiscardService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_OrderrQuickDiscard);
            }
        };
}]);
