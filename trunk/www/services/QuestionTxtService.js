/**
 * 服务:QuestionTxt，一对一问题回答
 */
app
    .service('QuestionTxtService', ["$rootScope","$q","$http","$log","ENV",function($rootScope,$q,$http,$log,ENV) {
        $log.debug("QuestionTxtService in");
        var aurl=ENV.api + "/QuestionTxt";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param obj QuestionTxt 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("QuestionTxtService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            update: function(obj,_page) {
				var tmpparam=this.makepostobj(obj,_page);
                $log.debug("QuestionTxtService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
